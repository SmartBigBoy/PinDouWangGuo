...
class PixelArtGenerator {
    ...
    initElements() {
        ...
        // 裁剪相关元素
        this.cropSection = document.getElementById('cropSection');
    ...
    initCropPreview() {
        ...
        const maxWidth = 600;
        const maxHeight = 400;
        let { width, height } = this.cropPreviewImage;
    ...
    drawCropOverlay() {
        ...
        this.cropCtx.fillStyle = 'rgba(102, 126, 234, 0.9)';
        const handleSize = 8;
        const handles = [
    ...
    handleCropMouseDown(e) {
        ...
        const handles = [
            ...
            { x: this.cropArea.x, y: this.cropArea.y + this.cropArea.height, type: 'sw', cursor: 'nesw-resize' },
        ...
        for (const handle of handles) {
            if (
                ...
                this.cropStart = { x, y };
                this.cropCanvas.style.cursor = handle.cursor;
    ...
    resizeCropArea(x, y) {
        const dx = x - this.cropStart.x;
        const dy = y - this.cropStart.y;
        const minSize = 50;
        
        switch (this.resizeHandle) {
            case 'nw':
                const nwWidth = this.cropArea.width - dx;
                const nwHeight = this.cropArea.height - dy;
                if (nwWidth >= minSize && nwHeight >= minSize) {
                    this.cropArea.width = nwWidth;
                    this.cropArea.height = nwHeight;
                    this.cropArea.x += dx;
                    this.cropArea.y += dy;
                }
                break;
            case 'ne':
                const neWidth = this.cropArea.width + dx;
                const neHeight = this.cropArea.height - dy;
                ...
                break;
            case 'sw':
                const swWidth = this.cropArea.width - dx;
                const swHeight = this.cropArea.height + dy;
                ...
                break;
            case 'se':
                const seWidth = this.cropArea.width + dx;
                const seHeight = this.cropArea.height + dy;
        ...
        this.cropArea.width = Math.min(this.cropArea.width, this.cropCanvas.width - this.cropArea.x);
        this.cropArea.height = Math.min(this.cropArea.height, this.cropCanvas.height - this.cropArea.y);
    ...
    dragCropArea(x, y) {
        ...
        newX = Math.max(0, Math.min(newX, this.cropCanvas.width - this.cropArea.width));
        newY = Math.max(0, Math.min(newY, this.cropCanvas.height - this.cropArea.height));
    ...
    setCropRatio(ratio) {
        ...
        const buttons = [
            this.freeRatioBtn,
            this.squareRatioBtn,
            this.fourThreeRatioBtn,
            this.sixteenNineRatioBtn
        ];
        ...
        switch (ratio) {
            case 'free':
                if (this.freeRatioBtn) this.freeRatioBtn.classList.add('active');
                break;
            case '1:1':
                if (this.squareRatioBtn) this.squareRatioBtn.classList.add('active');
                break;
            case '4:3':
                if (this.fourThreeRatioBtn) this.fourThreeRatioBtn.classList.add('active');
                break;
            case '16:9':
                if (this.sixteenNineRatioBtn) this.sixteenNineRatioBtn.classList.add('active');
                break;
        }
    }
    ...
    confirmCrop() {
        ...
        croppedCanvas.width = Math.round(this.cropArea.width * scaleX);
        croppedCanvas.height = Math.round(this.cropArea.height * scaleY);
